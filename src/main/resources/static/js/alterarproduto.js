...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!produtoData) {
        M.toast({ html: 'Nenhum produto selecionado para edição.', classes: 'yellow darken-2' });
    ...
    M.updateTextFields();
     ...
     function calcularCusto() {
        ...
        const modelagem = parseFloat(document.getElementById('modelagem').value) || 0;
        ...
        const malha = parseFloat(document.getElementById('malha').value) || 0;
        const costura = parseFloat(document.getElementById('costura').value) || 0;
        const etqmarca = parseFloat(document.getElementById('etqmarca').value) || 0;
        const etqcomp = parseFloat(document.getElementById('etqcomp').value) || 0;
        const tag = parseFloat(document.getElementById('tag').value) || 0;
        const refil = parseFloat(document.getElementById('refil').value) || 0;
        const elastico = parseFloat(document.getElementById('elastico').value) || 0;
        const saco = parseFloat(document.getElementById('saco').value) || 0;
        const fita = parseFloat(document.getElementById('fita').value) || 0;
        ...
        const custoTotal = peso + modelagem + desenvolvimento + corte + malha + costura +
                           etqmarca + etqcomp + tag + refil + elastico + saco + fita + embalagem + deslocamento;

        document.getElementById('custo').value = custoTotal.toFixed(4);
        M.updateTextFields();

        calcularValorVarejoEAtacado();
    }
    ...
    // Adiciona listeners para recalcular sempre que algum campo mudar
    const camposParaCalculo = [
        'peso', 'modelagem', 'desenvolvimento', 'corte', 'malha', 'costura',
        'etqmarca', 'etqcomp', 'tag', 'refil', 'elastico',
        'saco', 'fita', 'embalagem', 'deslocamento', 'fatormult'
    ];

    camposParaCalculo.forEach(id => {
        const campo = document.getElementById(id);
        if (campo) {
            campo.addEventListener('input', calcularCusto);
        }
    });

    // Calcula na carga inicial para garantir que tudo esteja consistente
    calcularCusto();
});
...
document.querySelector('.btn').addEventListener('click', async function (e) {
    ...
    if (camposFaltando.length > 0) {
        M.toast({ html: `Preencha os seguintes campos obrigatórios: ${camposFaltando.join(', ')}`, classes: 'yellow darken-2' });
    ...
    const produtoData = localStorage.getItem('produtoParaEditar');
    if (!produtoData) {
        M.toast({ html: 'Produto não encontrado.', classes: 'yellow darken-2' });
    ...
    if (!usuarioId) {
        M.toast({ html: 'ID do usuário não encontrado. Faça login novamente.', classes: 'yellow darken-2' });
