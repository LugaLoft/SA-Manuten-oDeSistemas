...
window.onload = function() {
    ...
    if (!vendedorData) {
        M.toast({ html: 'Nenhum vendedor selecionado para edição.', classes: 'yellow darken-2' });
    ...
    if (statusValue === 'ativo' || statusValue === 'desativado') {
...
document.querySelector('.btn').addEventListener('click', async function(e) {
    ...
    if (desconto < 0 || desconto > 100) {
        M.toast({ html: 'O desconto deve ser um valor entre 0 e 100.', classes: 'yellow darken-2' });
        return;
    }
    ...
    if (observacao.length > 250) {
        M.toast({ html: 'A observação deve ter no máximo 250 caracteres.', classes: 'yellow darken-2' });
        return;
    }
