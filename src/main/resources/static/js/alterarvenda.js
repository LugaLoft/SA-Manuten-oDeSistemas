...
async function editarVenda() {
  ...
  try {
    ...
    const response = await fetch(`../venda/${vendaParaEditar.id}`, {
      method: 'PUT',
      headers: {
    ...
    setTimeout(() => {
      ...
      window.location.href = '/buscarvenda';
    }, 80000);
  } catch (error) {
...
function carregarProdutosDaVenda(idVenda, token) {
    ...
    fetch(`/produtovenda/buscar?idVenda=${idVenda}`, {
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        }
...
async function enviarProdutosDaVenda() {
  try {
    ...
    const getResponse = await fetch(`/produtovenda/buscar?idVenda=${vendaId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`
      }
    ...
    if (produtosParaCriar.length > 0) {
      const postResponse = await fetch('../produtovenda', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    for (const antigo of produtosParaExcluir) {
      const deleteResponse = await fetch(`../produtovenda/${antigo.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  if (vendaParaEditar.id) {
    ...
    .then(produtos => {
      ...
      produtos.forEach(pv => {
        ...
        `;
        console.log('IDs de produtos salvos no localStorage:', idProduto);
        tabela.insertAdjacentHTML('beforeend', row);
      });
