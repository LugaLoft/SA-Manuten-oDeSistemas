...
function configurarAutocompletes() {
  ...
  setupProdutoAutocomplete('nome', '../nomeproduto/buscar', 'nome-suggestions', 'nome');
  setupVendedorAutocomplete('vendedor', '../vendedor/buscar', 'vendedor-suggestions', 'nome');
  setupPedidoAutocomplete('pedido', '../pedido/buscar', 'pedido-suggestions', 'id');
}
...
async function cadastrarVenda() {
  ...
  if (!formaPagamentoValue || !tipoEntregaValue) {
    M.toast({ html: 'Preencha todos os campos obrigatórios.', classes: 'yellow darken-2' });
    return;
  }
  ...
  try {
    ...
    if (result && result.id) {
      ...
      // Tenta enviar os produtos
      const produtosSalvos = await enviarProdutosDaVenda(); // ← aguarda resultado

      if (produtosSalvos) {
        M.toast({ html: 'Venda e produtos cadastrados com sucesso!', classes: 'green' });
        limparFormularioVenda();
      } else {
        throw new Error('Produtos não foram cadastrados corretamente.');
      }
    } else {
      throw new Error('Resposta da venda inválida.');
    }
...
function salvarProduto() {
  ...
  if (!nome || isNaN(quantidade) || isNaN(valor)) {
    M.toast({ html: 'Preencha os campos obrigatórios.', classes: 'yellow darken-2' });
    return;
  }
...
function carregarProdutosDoPedido(idPedido, token) {
    ...
    fetch(`/pedidoproduto/buscar?codPedido=${idPedido}`, {
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        }
...
async function enviarProdutosDaVenda() {
  try {
    ...
    if (produtoIds.length !== linhas.length) {
      throw new Error('IDs de produtos não correspondem às linhas da tabela.');
    }
    ...
    const response = await fetch('../produtovenda', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
...
function limparFormularioVenda() {
  ...
  // Limpa localStorage temporários
  localStorage.removeItem('venda_produto_ids');
  localStorage.removeItem('venda_cliente_id');
  localStorage.removeItem('venda_vendedor_id');
  localStorage.removeItem('venda_vendedor_desconto');
  localStorage.removeItem('venda_pedido_id');
