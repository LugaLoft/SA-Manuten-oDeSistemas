...
async function cadastrarPedido() {
  ...
  try {
    ...
    // Em vez de redirecionar, limpar campos e tabela
    limparFormulario();
    ...
    console.log(error);
    // Opcional: limpar localStorage mas não redirecionar
    limparLocalStorageExcetoTokenEUsuario();
  }
}
...
function limparFormulario() {
  ...
  document.getElementById('status').value = 'aguardando_aprovacao';

  // Atualizar selects com Materialize
  M.FormSelect.init(document.querySelectorAll('select'));
  M.updateTextFields();
...
function coletarDadosPedido(clienteId, usuarioId) {
  ...
  const dataBaseEnt = document.getElementById('dataentrega').value;

  const data = `${dataBase}T00:00:00-03:00`;
  const data_entrega = dataBaseEnt ? `${dataBaseEnt}T00:00:00-03:00` : null;
...
async function enviarProdutosDoPedido() {
  ...
  if (produtosPayload.length === 0) {
    M.toast({ html: 'Nenhum produto válido para enviar.', classes: 'yellow darken-2' });
    return;
  }
