...
async function editarPedido() {
  ...
  console.log('Dados enviados para atualização:', pedido);

  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`../pedido/${pedidoParaEditar.id}`, {
      method: 'PUT',
      headers: { 
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json' 
    ...
    if (result.id) {
      ...
      // Tenta enviar os produtos do pedido
      try {
        await enviarProdutosDoPedido();

        M.toast({ html: 'Pedido e produtos atualizados com sucesso!', classes: 'green' });

        setTimeout(() => {
          limparLocalStorageExcetoTokenEUsuario();
          window.location.href = '/buscarpedido';  // Ajuste a URL conforme seu projeto
        }, 2000);

      } catch (errorProdutos) {
        // Se falhar ao enviar produtos, não limpa nem redireciona
        M.toast({ html: `Erro ao atualizar produtos: ${errorProdutos.message}`, classes: 'red' });
      }
    }
...
function coletarDadosPedido(clienteId, usuarioId) {
  ...
  // Só adiciona se não for null/undefined
  if (clienteId) {
    pedido.cliente = { id: parseInt(clienteId) };
  }

  if (usuarioId) {
    pedido.usuario = { id: parseInt(usuarioId) };
  }
...
async function enviarProdutosDoPedido() {
  try {
    ...
    const getResponse = await fetch(`../pedidoproduto/pedido/${pedidoId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`
      }
    ...
    if (produtosParaCriar.length > 0) {
      const postResponse = await fetch('../pedidoproduto', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    for (const antigo of produtosParaExcluir) {
      const deleteResponse = await fetch(`../pedidoproduto/${antigo.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
