document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('buscarBtn').addEventListener('click', function(event) {
        ...
        if (!dataInicial || !dataFinal) {
            M.toast({ html: 'Por favor, preencha as datas inicial e final.', classes: 'yellow darken-2' });
            return;
        }

        if (new Date(dataInicial) > new Date(dataFinal)) {
            M.toast({ html: 'A data inicial deve ser anterior ou igual à data final.', classes: 'yellow darken-2' });
            return;
        }
