...
window.onload = function() {
    ...
    if (!usuarioData) {
        M.toast({ html: 'Nenhum usuário selecionado para edição.', classes: 'yellow darken-2' });
...
document.querySelector('.btn').addEventListener('click', async function(e) {
    ...
    if (usuario.length < 3 || usuario.length > 25) {
        M.toast({ html: 'O usuário deve ter entre 3 e 25 caracteres.', classes: 'yellow darken-2' });
        return;
    }

    if (usuario.includes(' ')) {
        M.toast({ html: 'O usuário não pode conter espaços.', classes: 'yellow darken-2' });
        return;
    }

    const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailPattern.test(email)) {
        M.toast({ html: 'Por favor, insira um e-mail válido.', classes: 'yellow darken-2' });
        return;
    }

    const senhaPattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^a-zA-Z0-9]).{8,}$/;
    if (senha && !senhaPattern.test(senha)) {
        M.toast({ html: 'A senha deve conter pelo menos uma letra maiúscula, uma letra minúscula, um caractere especial e ter no mínimo 8 caracteres.', classes: 'yellow darken-2' });
        return;
    }

    if (senha !== confirmarSenha) {
        M.toast({ html: 'As senhas não coincidem.', classes: 'yellow darken-2' });
        return;
    }
