document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (event) {
        ...
        if (!login || login.trim().length < 3) {
            M.toast({ html: 'Por favor, insira um email válido ou um nome de usuário (mínimo 3 caracteres).', classes: 'yellow darken-2' });
            return;
        }
    
        if (!password) {
            M.toast({ html: 'Por favor, insira sua senha.', classes: 'yellow darken-2' });
            return;
        }
